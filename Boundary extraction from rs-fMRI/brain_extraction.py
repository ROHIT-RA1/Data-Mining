...
def folderCreation():
 ...
 os.mkdir(os.path.join('./testPatient','Boundaries'))


def SubFolderCreation():
    # Creating Sub-Folders in Slices and Boundaries Folders
    for p in range(1,113):
       if(os.path.exists('./testPatient/Boundaries/OUT_{}.format(p)')==False):
        os.mkdir(os.path.join('./testPatient/Boundaries','OUT_{}'.format(p)))
